//requires jQuery, highslide, spin.js
...
function queryPhotosList(userId, albumId){
	...
	VK.api("photos.get",{uid:userId, aid:albumId},function(data) {
		if(data.response){
			...
			displayError("Не удалось получить список фотографий из выбранного альбома! Попробуйте перезагрузить приложение.", "globalErrorBox");
			console.log(data.error.error_msg);
			d.reject();
		}
	});
...
function fillAlbumsListBox(albums, listBoxId){
	...
	for(var i = 0; i < albums.length; i++){
		...
		var opt = new Option(title, albums[i].aid, false, false);
...
function userCheck(vkUid){
	...
	VK.api("users.get", {uids: vkUid, fields: "first_name, last_name, sex, bdate, city, country, relation"}, function(data) {
		if(data.response){
			var good = (function(dr){
				//check sex
				if(+dr.sex != 1)
					return false;
				
				//check bdate
				if(dr.bdate !== undefined){
					var d = dr.bdate.split(".");
					if(d.length == 3){
						var year = +d[2];
				...
				//check country
				if(dr.country !== undefined){
					if(+dr.country != myCountry)
						return false;
				}else{
					return false;
				}
			...
			if(good){
				d.resolve(data.response[0].first_name);
			}else{
				d.reject();
			}
		}else{
...
var AmApi__ = {
	...
	srcAlbumChanged: function() {
		...
		queryPhotosList(Settings.vkUserId, self.srcAlbumList.item(selIndex).value).done( function(photosList){
			self.srcPhotosNumEdit.value = photosList.length;
			
			self.revThumbSortChk.disabled = true;
			$("#thumbs_container").ThumbsViewer("addThumbList", photosList, self.revThumbSortChk.checked).done(function(){
				self.revThumbSortChk.disabled = false;
			});
	...
	dstAlbumChanged: function() {
		...
		queryPhotosList(Settings.vkUserId, self.dstAlbumList.item(selIndex).value).done( function(photosList){
...
d.done(function(){
	...
	queryAlbumsList().done(function(albums){
		//sort albums by name
		...
		fillAlbumsListBox(albums,"Form1_SrcAlbumList");
		fillAlbumsListBox(albums,"Form1_DstAlbumList");
		hideSpinner();
		AmApi__.welcomeCheck();
		/*userCheck(Settings.vkUserId).done(function(name){
			$("#addme").addme(name);
		});*/
	});
});
